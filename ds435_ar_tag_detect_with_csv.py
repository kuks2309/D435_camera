...
def main():
    ...
    try:
        ...
        last_save_time = None
        save_interval = 0.1  # Save every 0.1 seconds
        ...
        while True:
            ...
            # Write to CSV if recording (every 0.1 seconds)
            current_time = datetime.now()
            if recording and ids is not None and len(ids) > 0 and csv_writer is not None:
                # Check if enough time has passed since last save
                if last_save_time is None or (current_time - last_save_time).total_seconds() >= save_interval:
                    ...
                    for i in range(len(ids)):
                        ...
                        # Write row to CSV
                        csv_writer.writerow([
                            current_time.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3],  # timestamp
                            f"{elapsed_time:.3f}",  # elapsed time
                            marker_id,  # marker ID
                            f"{distance:.4f}",  # distance in meters
                            f"{tvec[0]:.4f}",  # X position in meters
                            f"{tvec[1]:.4f}",  # Y position in meters
                            f"{tvec[2]:.4f}",  # Z position in meters
                            f"{roll_deg:.2f}",  # roll in degrees
                            f"{pitch_deg:.2f}",  # pitch in degrees
                            f"{yaw_deg:.2f}"  # yaw in degrees
                        ])
            ...
            if key == ord(' '):
                if not recording:
                    ...
                    folder_name = recording_start_time.strftime('%Y-%m-%d')
                    os.makedirs(folder_name, exist_ok=True)

                    # Create CSV file with start time
                    file_name = recording_start_time.strftime('%H-%M-%S') + '.csv'
